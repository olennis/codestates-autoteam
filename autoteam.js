function myFunction() {
  var sheetName = "000기 000 Team"; //${oo}기 ${First || Final} Team //이 이름으로 시트가 생성됩니다.
  var sheet = SpreadsheetApp.getActive();
  var obj = {};
  var sheet = SpreadsheetApp.getActiveSheet();
  var students = sheet.getRange("A1:A60").getValues(); //숫자만 바꾸면 됩니다
  var favorite = sheet.getRange("D1:F60").getValues(); //숫자만 바꾸면 됩니다
  var difficult = sheet.getRange("G1:I60").getValues(); //숫자만 바꾸면 됩니다
  ...
  const autoTeam = (data) => {
    ...
    const makeFitTeam = () => {
      ...
      for (let key in data) {
        if (isFitTeam(key)) {
          ...
          isCross(key, data[key].Favorite[0]) &&
          isCross(key, data[key].Favorite[1]) &&
          isCross(data[key].Favorite[1], data[key].Favorite[0])
        ) {
          madeTeam.push(
            [key, data[key].Favorite[0], data[key].Favorite[1]].sort()
          );
        } else if (
          isCross(key, data[key].Favorite[0]) &&
          isCross(key, data[key].Favorite[2]) &&
          isCross(data[key].Favorite[2], data[key].Favorite[0])
        ) {
          madeTeam.push(
            [key, data[key].Favorite[0], data[key].Favorite[2]].sort()
          );
        } else if (
          isCross(key, data[key].Favorite[1]) &&
          isCross(key, data[key].Favorite[2]) &&
          isCross(data[key].Favorite[2], data[key].Favorite[1])
        ) {
          madeTeam.push(
            [key, data[key].Favorite[1], data[key].Favorite[2]].sort()
          );
        }
      ...
      for (let i = remainArr.length - 1; i >= 0; i--) {
        for (let key in team) {
          if (
            isCross(remainArr[i], team[key][0]) ||
            isCross(remainArr[i], team[key][1]) ||
            isCross(remainArr[i], team[key][2])
          ) {
            if (
              isFav(team[key][0], remainArr[i]) &&
              isFav(team[key][1], remainArr[i]) &&
              isFav(team[key][2], remainArr[i])
            ) {
    ...
    const makeTeam = () => {
      ...
      console.log(
        sortArr,
        "배치되지 않은 사람들입니다. 선호도 점수 순으로 들어가요"
      );
      console.log(
        team,
        "사전에 말을 맞춘 사람들입니다. 선호도 점수에 영향을 받지 않습니다."
      );
      const peopleNum = Object.keys(data).length; //총 인원 수
      if (peopleNum % 4 === 0) {
        console.log(
          `총 인원 : ${peopleNum}, 4명 팀 : ${parseInt(
            peopleNum / 4
          )}, 3명 팀 : 0`
        );
        ...
        for (let i = 0; i < copySort.length; i += 1) {
          ...
          for (let j = Math.max(...Object.keys(team)); j >= 1; j--) {
            //team 객체 순회
            if (
              isCross(currentStudent, team[j][0]) ||
              isCross(currentStudent, team[j][1]) ||
              isCross(currentStudent, team[j][2])
            ) {
              if (team[j].length < 4) {
                cross = true;
                crossTeam = j;
              ...
              team[j].includes(data[currentStudent].Favorite[0]) ||
              team[j].includes(data[currentStudent].Favorite[1]) ||
              team[j].includes(data[currentStudent].Favorite[2])
            ) {
              if (team[j].length < 4) {
                favTeam = j;
                myFav = true;
          ...
          if (cross && !isTeamDiff(team[crossTeam], currentStudent)) {
            ...
            if (makeHead !== 0) {
              ...
              for (let key in team) {
                if (
                  team[key].length < 4 &&
                  !isTeamDiff(team[key], currentStudent) &&
                  !assignedArr.includes(currentStudent)
                ) {
